    ...
    return (
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
            <div className="space-y-1.5">
                <label className="font-mono text-[11px] uppercase tracking-wider text-ink-500 block">Full Name</label>
                <input
                    type="text"
                ...
                />
                {errors.name && <span className="text-rose-500 text-xs mt-1 block font-body">{errors.name.message}</span>}
            </div>
            ...
            <div className="flex items-center justify-center text-sm pt-1">
                <span className="text-ink-500 mr-1.5">Already have an account?</span>
                <Link to="/login" className="text-amber-500 hover:text-amber-600 font-medium transition-colors">Sign in</Link>
            </div>
        </form>
    );
};
